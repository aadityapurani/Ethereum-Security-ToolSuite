# Tool to convert EVM op-code ASM back to run-time. 
# Input format: instruction_num instruction value
# Easily tweakable based on input provided
...
# https://gist.github.com/ajsantander/60bd8d6f88725663f89a67a7c51672c7
# Save it as BasicToken.asm
with open('BasicToken.asm', 'r') as f:
    line = f.readline()
    cnt = 1
    while line:
        instr = line.split(' ')[1]
        try:
            ...
            val = ''
        instr = instr.strip('\n')
        opcodez = hex(getKeysByValue(opcodes, instr)[0])[2:]
        runtime_bytecode += opcodez + val
        line = f.readline()
        cnt +=1
print "[+] Bytecode "
print runtime_bytecode.strip('\n')
with open('converted.bytecode', 'w') as f:
    f.write(runtime_bytecode.strip('\n'))
